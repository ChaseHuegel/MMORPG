...
{
    public PortalAuthenticationHandler(
    ...
    protected override async Task<HandleRequestResult> HandleRemoteAuthenticateAsync()
    {
        ...
        httpClient.DefaultRequestHeaders.TryAddWithoutValidation(
            "Cookie",
            "MMO.Portal.Login=" + cookie
        );
        var validateResponse = await httpClient.PostAsync(
            "https://localhost:7297/api/Session/Validate",
            null
        );

        if (!validateResponse.IsSuccessStatusCode)
            return HandleRequestResult.Fail(
                new UnauthorizedAccessException(validateResponse.StatusCode.ToString())
            );

        var claims = new Claim[] { new Claim(ClaimTypes.NameIdentifier, "test") };
